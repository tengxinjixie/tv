var rule = {
  title: '极光影院',
  host: 'https://m.hellociqryx6e.com/',
  ...
  二级: `js:
    ...
    let key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    let kjson = JSON.parse(request('https://m.hellociqryx6e.com/api/mw-movie/anonymous/video/detail?id='+kid,{headers:{
       'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36',
        'Accept': 'application/json, text/plain, */*',
        'deviceId': '63ffad23-a598-4f96-85d7-7bf5f3e4a0a2',
        'sign': key,
    ...
    let kurls = kjson.episodeList.map(function(it) => {
      return it.name + "$" + "https://m.hellociqryx6e.com/vod/play/"+kid+"/sid/"+it.nid
    }).join('#');
    VOD = {
      vod_id: kid,
